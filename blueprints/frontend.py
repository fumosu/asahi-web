...
async def home():
    ...
    ap = await glob.db.fetchrow('SELECT pp, users.name, ptime FROM scores_ap LEFT JOIN users ON scores_ap.uid = users.id LEFT JOIN maps ON scores_ap.md5 = maps.md5 WHERE users.priv & 2 AND NOT users.priv & 1024 AND maps.status = 2 AND scores_ap.status = 2 AND scores_ap.mode = 0 ORDER BY pp DESC LIMIT 1')
    try:
